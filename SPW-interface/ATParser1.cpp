...
int ATParser1::write(const char *data, int size)
{
    int i = 0;
    for ( ; i < size; i++) {
        if (putc(data[i]) < 0) {
...
int ATParser1::read(char *data, int size)
{
    int i = 0;
    for ( ; i < size; i++) {
        int c = getc();
...
int ATParser1::vprintf(const char *format, va_list args)
{
...
int ATParser1::vscanf(const char *format, va_list args)
{
...
bool ATParser1::vsend(const char *command, va_list args)
{
...
bool ATParser1::vrecv(const char *response, va_list args)
{
    ...
    while (response[0]) {
        ...
        while (true) {
            ...
            _buffer[offset + j++] = c;
            _buffer[offset + j] = 0;
//			debug_if(dbg_on, "%02X.", c); // DEBUG RSR
...
int ATParser1::printf(const char *format, ...)
{
    va_list args;
...
int ATParser1::scanf(const char *format, ...)
{
    va_list args;
    va_start(args, format);
...
bool ATParser1::send(const char *command, ...)
{
    va_list args;
...
bool ATParser1::recv(const char *response, ...)
{
    va_list args;
